...
func saveFile() {
	...
	saveStr := Map2String(saveMap)

	fileLock.Lock()
	defer fileLock.Unlock()
	log.Println(saveStr)
	WriteFile("./tmp.txt", saveStr)
}

// loadFile 在此模块一开始运行的时候加载缓存文件
func loadFile() {
	fileLock.Lock()
	defer fileLock.Unlock()

	fileStr, _ := ReadFile("./tmp.txt")
	loadMap := String2Map(fileStr)
